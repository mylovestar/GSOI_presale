...
contract UFXToken is BEP20 {
    ...
    // Liquify rate % of transfer tax. (default 5% x 93% = 4.65% of total amount).
    uint16 public liquifyRate = 93; // INMUTABLE

    // UFXdev Fee address
    address public devAddress;

    // Min amount to liquify. (default 400 UFX)
     ...
     */
    constructor(address _devAddress, address _PCSRouter) public BEP20("United Farmers X", "UFX") {
    ...
    function swapAndLiquify() private lockTheSwap transferTaxFree {
        ...
        if (contractTokenBalance >= minAmountToLiquify) {
            ...
            uint256 initialBalance = BUSD.balanceOf(address(this));//ex) 150 eth

            // swap tokens for BUSD
            swapTokensForBUSD(half); //ex) 50eth
    ...
    function swapTokensForBUSD(uint256 tokenAmount) private {
        ...
        uint initialBalance = BUSD.balanceOf(operator());
        // make the swap
        PCSRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
        ...
        uint newBalance = BUSD.balanceOf(operator()).sub(initialBalance);
        BUSD.transferFrom(operator(), address(this), newBalance);
    }

    /// @dev Add liquidity
    function addLiquidity(uint256 tokenAmount, uint256 busdAmount) private {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(PCSRouter), busdAmount);

        // add the liquidity
        PCSRouter.addLiquidity(
            address(this),
            busdTokenAddress,
            tokenAmount,
            busdAmount,
